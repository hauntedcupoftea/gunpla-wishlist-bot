...
(async () => {
  try {
    ...
    if (env.GUILD_ID) {
      ...
      console.log(`[bot] Registered ${data.length} guild commands (${env.GUILD_ID}).`);
    } else {
      const data = await rest.put(
...
// Dynamically import the API so that if Elysia deps are unavailable in a
// bot-only deployment, the bot still starts cleanly.
try {
