...
const data = new SlashCommandBuilder()
  ...
  .addIntegerOption((option) =>
    option
      .setName("jpy_price")
      .setDescription(
        "Price of the kit in JPY. Use of market value is discouraged, unless it is not sold in Japan.",
      )
      .setRequired(true)
  )
  .addStringOption((option) =>
    option
      ...
      .setDescription(
        "Availability status of the kit. You may enter Sources here if you wish.",
      )
      .setRequired(true)
  )
  .addStringOption((option) =>
    option
      ...
      .setDescription(
        "The release date of the kit (optional). Month Year format is appreciated (eg. September 2024)",
      )
      .setRequired(false)
  )
  .addStringOption((option) =>
...
async function execute(interaction) {
  ...
  try {
    ...
    await interaction.reply(
      `Successfully added the kit **${newKit.product_name}**!`,
    );
  } catch (error) {
    console.error(error);
    await interaction.reply({
      content: "There was an error while adding the kit.",
      ephemeral: true,
    });
  }
}
