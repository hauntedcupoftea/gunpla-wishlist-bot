  ...
  switch (subcommand) {
    case "add":
      try {
        ...
        console.error(error);
        await interaction.reply({
          content: "There was an error while adding the kit to your wishlist.",
          ephemeral: true,
        });
      }
      break;
    case "remove":
      ...
      if (result.count > 0) {
        ...
        await interaction.reply({
          content: `Kit **${kitId}** not found in your wishlist.`,
          ephemeral: true,
        });
      }
      break;
    case "view":
      ...
      if (wishlists.length > 0) {
        const wishlistItems = wishlists.map((item) =>
          `- **${item.kit.product_name}**`
        ).join("\n");
