...
async function assertForumThread(
  ...
  try {
    ...
    if (
      ...
      await interaction.reply({
        content:
          "⚠️ Group buy commands must be used inside a **forum thread**.",
        flags: MessageFlags.Ephemeral,
      });
      return false;
    }
...
export default {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        .setName("setdomesticship")
        .setDescription(
          "Set domestic shipping cost — seller to forwarding warehouse (organiser/buyer only). Use 0 to clear.",
        )
        .addIntegerOption((o) =>
          o
            .setName("domestic_cost")
            .setDescription(
              "Domestic shipping cost in JPY (0 = seller covers it)",
            )
            .setRequired(true)
            .setMinValue(0)
        )
            ...
            .addStringOption((o) =>
              o
                ...
                .addChoices(
                  ...
                  { name: "📦 Domestic shipping", value: "DOMESTIC_SHIPPING" },
  ...
  async execute(interaction) {
    ...
    if (subcommand === "addkit") {
      ...
      try {
        ...
        await interaction.reply(
          withOverrideNote(
            `Added **${quantity}× ${kitName}** (${weight}g each). Total weight: ${
              updated?.totalWeight ?? "?"
            }g.`,
            adminOverride,
          ),
    ...
    if (subcommand === "setprice") {
      ...
      const _updated = await GbService.updateFinancials(gb.id, {
        costPrice,
        inrConversionRate,
      });
      ...
      await interaction.reply(
        withOverrideNote(
          `Kit cost set to **¥${costPrice.toLocaleString()}** at **₹${inrConversionRate}/¥**. ` +
            `Recalculated across **${claimCount}** active claim${
              claimCount !== 1 ? "s" : ""
            }.`,
          adminOverride,
        ),
    ...
    if (subcommand === "setdomesticship") {
      ...
      const domesticCost = interaction.options.getInteger(
        "domestic_cost",
        true,
      );
      await GbService.updateFinancials(gb.id, {
        domesticShippingCost: domesticCost === 0 ? null : domesticCost,
      });
      const claimCount = gb.claims.filter(
      ...
      await interaction.reply(
        withOverrideNote(
          domesticCost === 0
            ? `Domestic shipping cost cleared.`
            : `Domestic shipping set to **¥${domesticCost.toLocaleString()}**. Recalculated across **${claimCount}** claim${
              claimCount !== 1 ? "s" : ""
            }.`,
          adminOverride,
        ),
      ...
      await interaction.reply(
        withOverrideNote(
          `Shipping set to **¥${shippingCost.toLocaleString()}** for **${shippingWeight}g**. ` +
            `Recalculated across **${claimCount}** claim${
              claimCount !== 1 ? "s" : ""
            }.`,
          adminOverride,
        ),
    ...
    if (subcommand === "claim") {
      if (gb.status !== "OPEN") {
        await interaction.reply({
          content: `This group buy is **${
            gb.status.replace(/_/g, " ")
          }** and is not accepting new claims.`,
          flags: MessageFlags.Ephemeral,
        });
        return;
      }
    ...
    if (subcommandGroup === "claims") {
      ...
      if (subcommand === "view") {
        ...
        if (isOwner) {
          ...
          const lines = withClaims.map((s) => {
            const slotLabel = slots.filter((x) =>
                x.kitId === s.kitId
              ).length > 1
      ...
      if (subcommand === "manage") {
        ...
        if (uncontested.length > 0) {
          const btn = new ButtonBuilder()
            ...
            .setLabel(
              `Confirm ${uncontested.length} uncontested claim${
                uncontested.length !== 1 ? "s" : ""
              }`,
            )
        ...
        if (contested.length === 0) {
                  ...
                  .map(
                    (k) =>
                      `  · ${k.kit.product_name} #${k.slotNumber} → <@${
                        k.claims[0].userId
                      }> [${CLAIM_STATUS_LABEL[k.claims[0].status]}]`,
                  )
                  .join("\n")
              }`,
            );
          ...
          const canAdvance = unclaimed.length === 0 &&
        ...
        if (components.length > 0) {
          ...
          collector.on("collect", async (i) => {
            if (i.isButton() && i.customId === "claims:confirm_uncontested") {
              ...
              await i.reply({
                content: `✅ Confirmed ${count} uncontested claim${
                  count !== 1 ? "s" : ""
                }.`,
                flags: MessageFlags.Ephemeral,
              });
            ...
            if (
              ...
              await i.reply({
                content: remaining > 0
                  ? `✅ Conflict resolved. **${remaining}** slot${
                    remaining !== 1 ? "s" : ""
      ...
      if (subcommand === "unclaim") {
        ...
        await interaction.reply({
          content:
            `Cancelled your claim for **${claim.groupBuyKit.kit.product_name}**.`,
          flags: MessageFlags.Ephemeral,
        });
        return;
      }
      ...
      if (subcommand === "pay") {
        ...
        await interaction.reply({
          content: `✅ Your **${stage}** payment has been reported. ` +
      ...
      if (subcommand === "confirm") {
        ...
        try {
          ...
          const isLastStage = stage === "CUSTOMS" ||
