...
export default {
  ...
  async autocomplete(interaction) {
    ...
    if (subcommand === "add") {
      ...
      await interaction.respond(results.map((k: KitSearchResult) => ({ name: k.product_name, value: k.id })));
      ...
      await interaction.respond(
        filtered.slice(0, 5).map((w: WishlistWithKit) => ({ name: w.kit.product_name, value: w.kitId })),
      );
    } else {
      await interaction.respond([]);
    }
  ...
  async execute(interaction) {
    ...
    if (subcommand === "view") {
      ...
      if (wishlist.length === 0) {
        await interaction.reply({ content: "Your wishlist is empty.", flags: MessageFlags.Ephemeral });
        return;
      }

      const lines = wishlist.map((w: WishlistWithKit) => `- **${w.kit.product_name}** (${fmtJpy(w.kit.jpy_price)})`);
      await interaction.reply({
        content: `Your wishlist (${wishlist.length} kit${wishlist.length !== 1 ? "s" : ""}):\n${lines.join("\n")}`,
        flags: MessageFlags.Ephemeral,
      });
    }
  },
