...
import type { GroupBuyStatus, PaymentStage, GroupBuyClaim } from "../../../generated/client.ts";
...
import { withOverrideNote, CLAIM_STATUS_LABEL, nextClaimStatus } from "../../lib/util.ts";
...
function checkOwner(
  ...
  if (gb.ownerId === interaction.user.id) return { allowed: true, adminOverride: false };
...
async function replyLocked(interaction: ChatInputCommandInteraction): Promise<void> {
  await interaction.reply({
    content: "🔒 This group buy is locked by an administrator. No actions can be taken.",
    flags: MessageFlags.Ephemeral,
  });
}

async function replyNotFound(interaction: ChatInputCommandInteraction): Promise<void> {
  await interaction.reply({
    content: "No group buy found for this thread. Use `/gb create` to start one.",
    flags: MessageFlags.Ephemeral,
  });
}
...
async function assertForumThread(
  ...
  try {
    ...
    if (channel?.isThread() && !parent && channel.parentId) {
      parent = (await interaction.client.channels.fetch(channel.parentId)) as any;
    }
...
export default {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub.setName("create").setDescription("Create a group buy in this forum thread"),
    )
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((o) =>
          o.setName("kit").setDescription("Kit to add").setRequired(true).setAutocomplete(true),
        )
        .addIntegerOption((o) =>
          o
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((o) =>
          o.setName("kit").setDescription("Kit to remove").setRequired(true).setAutocomplete(true),
        )
        .addIntegerOption((o) =>
          o
            .setName("quantity")
    ...
    .addSubcommand((sub) =>
      sub
        .setName("setbuyer")
        .setDescription("Set the payment aggregator for this group buy (organiser only)")
    ...
    .addSubcommand((sub) =>
      sub
        .setName("setreceiver")
        .setDescription("Set the parcel receiver for this group buy (organiser only)")
        .addUserOption((o) =>
    ...
    .addSubcommand((sub) =>
      sub
        .setName("transfer")
        .setDescription("Transfer group buy ownership to another member (organiser only)")
        .addUserOption((o) =>
    ...
    .addSubcommand((sub) =>
      sub
        .setName("setprice")
        .setDescription("Set kit cost price and JPY→INR conversion rate (organiser/buyer only)")
        .addIntegerOption((o) =>
          o
            .setName("cost_price")
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .setDescription(
          "Set domestic warehouse handling fee (organiser/buyer only). " +
            "Split by MSRP proportion. Use 0 to clear.",
        )
        .addIntegerOption((o) =>
          o
            .setName("warehouse_cost")
            .setDescription("Total warehouse fee in JPY (0 = none)")
            .setRequired(true)
            .setMinValue(0),
        ),
    )
      ...
      sub
        ...
        .addIntegerOption((o) =>
          o
            ...
            .setDescription("Actual courier-weighed parcel weight in grams (for your records)")
            .setRequired(true),
        )
        .addStringOption((o) =>
          o.setName("tracking_number").setDescription("Tracking number").setRequired(false),
        )
        .addStringOption((o) =>
          o.setName("tracking_url").setDescription("Tracking URL").setRequired(false),
        ),
    )
    ...
    .addSubcommand((sub) =>
      sub
        .setName("setcustoms")
        .setDescription("Set customs/import duties and tracking (organiser/buyer only)")
        .addIntegerOption((o) =>
          o
    ...
    .addSubcommand((sub) =>
      sub.setName("info").setDescription("View group buy overview for this thread"),
    )

    .addSubcommand((sub) =>
      sub
        .setName("balance")
        .setDescription("View your personal cost breakdown and payment status for this group buy"),
    )
    ...
    .addSubcommandGroup((group) =>
      group
        ...
        .addSubcommand((sub) =>
          sub.setName("view").setDescription("View claims (organiser sees all; members see their own)"),
        )
        .addSubcommand((sub) =>
          sub
            .setName("manage")
            .setDescription("Resolve conflicts and confirm uncontested claims (organiser only)"),
        )
        .addSubcommand((sub) =>
          sub
            .setName("unclaim")
        ...
        .addSubcommand((sub) =>
          sub
            .setName("transfer")
            .setDescription("Transfer a claim to another member (organiser only)")
            .addStringOption((o) =>
              o
                .setName("claim")
            ...
            .addUserOption((o) =>
              o.setName("user").setDescription("New claimant").setRequired(true),
            ),
        )
        .addSubcommand((sub) =>
          sub
            ...
            .addStringOption((o) =>
              o
                .setName("note")
                .setDescription("Optional note (e.g. UTR number, screenshot link)")
                .setRequired(false),
            ),
        )
        .addSubcommand((sub) =>
          sub
            .setName("confirm")
            .setDescription("Confirm a member's reported payment (organiser/buyer only)")
            .addStringOption((o) =>
              o
                .setName("claim")
  ...
  async autocomplete(interaction) {
    ...
    try {
      if (subcommand === "addkit") {
        ...
        const existing = await import("../../lib/prisma.ts").then(({ prisma }) =>
      ...
      if (subcommand === "transfer" || subcommand === "confirm") {
        ...
        const options = await GbService.getAllClaimOptions(interaction.channelId, focused);
        await interaction.respond(options);
        return;
      }
    } catch {
  ...
  async execute(interaction) {
    ...
    if (subcommand === "info") {
      ...
      if (!gb) { await replyNotFound(interaction); return; }
    ...
    if (subcommand === "balance") {
      ...
      const claims = await GbService.getUserClaimsInGroupBuy(gb.id, interaction.user.id);
      const summaries = await GbService.getGroupBuySummary(gb.id);
      const mySummaries = summaries.filter((s) => s.userId === interaction.user.id);
    ...
    if (subcommand === "addkit") {
      ...
      if (!allowed) {
        await interaction.reply({ content: "Only the organiser can add kits.", flags: MessageFlags.Ephemeral });
        return;
      }
      ...
      try {
        const slots = await GbService.addKitToGroupBuy({
          groupBuyId: gb.id, kitId, weightGrams: weight, quantity,
        });
        const kitName = slots[0] ? (await import("../../lib/prisma.ts").then(({ prisma }) =>
          prisma.kit.findUnique({ where: { id: kitId }, select: { product_name: true } })
        ))?.product_name ?? kitId : kitId;
    ...
    if (subcommand === "removekit") {
      const { allowed, adminOverride } = checkOwner(interaction, gb);
      if (!allowed) {
        await interaction.reply({ content: "Only the organiser can remove kits.", flags: MessageFlags.Ephemeral });
        return;
      }
    ...
    if (subcommand === "setbuyer") {
      const { allowed, adminOverride } = checkOwner(interaction, gb);
      if (!allowed) {
        await interaction.reply({ content: "Only the organiser can set the buyer.", flags: MessageFlags.Ephemeral });
        return;
      }
      const user = interaction.options.getUser("user", true);
      await GbService.setGroupBuyBuyer(gb.id, user.id);
      await interaction.reply(withOverrideNote(`Set <@${user.id}> as the payment handler.`, adminOverride));
      return;
    }
    ...
    if (subcommand === "setreceiver") {
      const { allowed, adminOverride } = checkOwner(interaction, gb);
      if (!allowed) {
        await interaction.reply({ content: "Only the organiser can set the receiver.", flags: MessageFlags.Ephemeral });
        return;
      }
      const user = interaction.options.getUser("user", true);
      await GbService.setGroupBuyReceiver(gb.id, user.id);
      await interaction.reply(withOverrideNote(`Set <@${user.id}> as the parcel receiver.`, adminOverride));
      return;
    }
    ...
    if (subcommand === "transfer") {
      const { allowed, adminOverride } = checkOwner(interaction, gb);
      if (!allowed) {
        await interaction.reply({ content: "Only the organiser can transfer ownership.", flags: MessageFlags.Ephemeral });
        return;
      }
      const user = interaction.options.getUser("user", true);
      await GbService.transferGroupBuyOwnership(gb.id, user.id);
      await interaction.reply(withOverrideNote(`Transferred ownership to <@${user.id}>.`, adminOverride));
      return;
    }

    // ── /gb setstatus ──────────────────────────────────────────────────────
    if (subcommand === "setstatus") {
      const newStatus = interaction.options.getString("status", true) as GroupBuyStatus;
      const isLockOp  = newStatus === "LOCKED" || gb.status === "LOCKED";

      if (isLockOp && !isAdmin(interaction)) {
        await interaction.reply({
          content: "Only a server administrator can lock or unlock a group buy.",
          flags: MessageFlags.Ephemeral,
        });
        return;
      }
      ...
      const { allowed, adminOverride } = checkOwner(interaction, gb);
      if (!allowed) {
        await interaction.reply({ content: "Only the organiser can change the status.", flags: MessageFlags.Ephemeral });
        return;
      }

      await GbService.setGroupBuyStatus(gb.id, newStatus);
      await interaction.reply(
        withOverrideNote(`Status updated to **${newStatus.replace(/_/g, " ")}**.`, adminOverride),
      );
      return;
    }
    ...
    if (subcommand === "setprice") {
      ...
      if (!allowed) {
        await interaction.reply({ content: "Only the organiser or buyer can set the price.", flags: MessageFlags.Ephemeral });
        return;
      }
      ...
      const updated = await GbService.updateFinancials(gb.id, { costPrice, inrConversionRate });
      const claimCount = gb.claims.filter((c: GroupBuyClaim) => c.status !== "CANCELLED").length;
    ...
    if (subcommand === "setwarehouse") {
      const { allowed, adminOverride } = checkOwnerOrBuyer(interaction, gb);
      if (!allowed) {
        await interaction.reply({ content: "Only the organiser or buyer can set the warehouse fee.", flags: MessageFlags.Ephemeral });
        return;
      }

      const warehouseCost = interaction.options.getInteger("warehouse_cost", true);
    ...
    if (subcommand === "setshipping") {
      const { allowed, adminOverride } = checkOwnerOrBuyer(interaction, gb);
      if (!allowed) {
        await interaction.reply({ content: "Only the organiser or buyer can set shipping.", flags: MessageFlags.Ephemeral });
        return;
      }

      const shippingCost   = interaction.options.getInteger("shipping_cost", true);
      const shippingWeight = interaction.options.getInteger("shipping_weight", true);
    ...
    if (subcommand === "setcustoms") {
      const { allowed, adminOverride } = checkOwnerOrBuyer(interaction, gb);
      if (!allowed) {
        await interaction.reply({ content: "Only the organiser or buyer can set customs.", flags: MessageFlags.Ephemeral });
        return;
      }
      ...
      if (trackingUrl) await GbService.updateTracking(gb.id, { customsTrackingUrl: trackingUrl });
    ...
    if (subcommand === "summary") {
      ...
      if (!allowed) {
        await interaction.reply({ content: "Only the organiser or buyer can post the summary.", flags: MessageFlags.Ephemeral });
        return;
      }

      const summaries = await GbService.getGroupBuySummary(gb.id);
      const confirmed = summaries.filter((s) =>
        !["PENDING", "CANCELLED"].includes(s.status),
      );

      if (confirmed.length === 0) {
        await interaction.reply({
          content: "No confirmed claims yet. Resolve conflicts with `/gb claims manage` first.",
          flags: MessageFlags.Ephemeral,
        });
        return;
      }
    ...
    if (subcommand === "claim") {
      ...
      try {
        const claim = await GbService.claimKit(gb.id, kitId, interaction.user.id);
        await interaction.reply({
          content:
    ...
    if (subcommandGroup === "claims") {
      ...
      // /gb claims view ────────────────────────────────────────────────────
      if (subcommand === "view") {
        const isOwner = gb.ownerId === interaction.user.id || isAdmin(interaction);
        ...
        if (isOwner) {
          ...
          if (withClaims.length === 0) {
            await interaction.reply({ content: "No active claims yet.", flags: MessageFlags.Ephemeral });
            return;
          }
          ...
          const lines = withClaims.map((s) => {
            ...
            const claimLines = s.claims
              .map((c) => `  · <@${c.userId}> — ${CLAIM_STATUS_LABEL[c.status] ?? c.status}`)
              .join("\n");
          ...
          const summaries = await GbService.getGroupBuySummary(gb.id);
          const mine = summaries.filter((s) => s.userId === interaction.user.id);

          if (mine.length === 0) {
            await interaction.reply({ content: "You have no active claims.", flags: MessageFlags.Ephemeral });
            return;
          }
      ...
      if (subcommand === "manage") {
        ...
        if (!allowed) {
          await interaction.reply({ content: "Only the organiser can manage claims.", flags: MessageFlags.Ephemeral });
          return;
        }

        const slots = await GbService.getSlotsWithClaimCounts(gb.id);
        const contested      = slots.filter((s) => s.claims.length > 1);
        const uncontested    = slots.filter((s) => s.claims.length === 1 && s.claims[0].status === "PENDING");
        const resolvedSlots  = slots.filter((s) => s.claims.length === 1 && s.claims[0].status !== "PENDING");
        ...
        if (uncontested.length > 0) {
          const btn = new ButtonBuilder()
            ...
            .setLabel(`Confirm ${uncontested.length} uncontested claim${uncontested.length !== 1 ? "s" : ""}`)
        ...
        if (contested.length === 0) {
          ...
          if (resolvedSlots.length > 0) {
            parts.push(
              `✅ **Resolved:**\n${resolvedSlots
                .map((k) => `  · ${k.kit.product_name} #${k.slotNumber} → <@${k.claims[0].userId}> [${CLAIM_STATUS_LABEL[k.claims[0].status]}]`)
                .join("\n")}`,
            );
          }
          ...
          const canAdvance = unclaimed.length === 0 && resolvedSlots.length > 0 && gb.status === "OPEN";
        ...
        if (components.length > 0) {
          ...
          collector.on("collect", async (i) => {
            ...
            if (i.isStringSelectMenu() && i.customId.startsWith("claims:resolve:")) {
              const gbkId         = i.customId.split(":")[2];
              const winnerClaimId = i.values[0];

              await GbService.resolveContestedSlot(gbkId, winnerClaimId);

              const stillContested = await GbService.getSlotsWithClaimCounts(gb.id);
              const remaining = stillContested.filter((s) => s.claims.length > 1).length;
      ...
      if (subcommand === "unclaim") {
        ...
        if (!claim) {
          await interaction.reply({ content: "Claim not found.", flags: MessageFlags.Ephemeral });
          return;
        }
        ...
        if (!isOwner && !isClaimer) {
          await interaction.reply({ content: "You can only cancel your own claims.", flags: MessageFlags.Ephemeral });
          return;
        }
      ...
      if (subcommand === "transfer") {
        const { allowed, adminOverride } = checkOwner(interaction, gb);
        if (!allowed) {
          await interaction.reply({ content: "Only the organiser can transfer claims.", flags: MessageFlags.Ephemeral });
          return;
        }
      ...
      if (subcommand === "pay") {
        const claimId = interaction.options.getString("claim", true);
        const stage   = interaction.options.getString("stage", true) as PaymentStage;
        ...
        if (!claim || claim.userId !== interaction.user.id) {
          await interaction.reply({ content: "Claim not found or not yours.", flags: MessageFlags.Ephemeral });
          return;
        }

        await GbService.reportPayment(claimId, interaction.user.id, stage, note);
      ...
      if (subcommand === "confirm") {
        const { allowed, adminOverride } = checkOwnerOrBuyer(interaction, gb);
        if (!allowed) {
          await interaction.reply({ content: "Only the organiser or buyer can confirm payments.", flags: MessageFlags.Ephemeral });
          return;
        }
        ...
        try {
          const { claim } = await GbService.confirmPaymentStage(
            claimId, interaction.user.id, stage, note,
          );
