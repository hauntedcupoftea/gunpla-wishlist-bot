...
import type { Wishlist } from "../../../generated/models.ts";
...
export default {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((o) =>
          o.setName("product_name").setDescription("Full kit name").setRequired(true),
        )
        .addIntegerOption((o) =>
          o.setName("jpy_price").setDescription("MSRP in JPY").setRequired(true),
        )
        .addIntegerOption((o) =>
          o
        ...
        .addStringOption((o) =>
          o.setName("availability").setDescription("Source / availability").setRequired(true),
        )
        .addStringOption((o) =>
          o
            .setName("item_code")
            .setDescription("Supplier item code (e.g. BAN123456). Defaults to product name.")
            .setRequired(false),
        )
        .addStringOption((o) =>
          o
        ...
        .addStringOption((o) =>
          o.setName("stock_status").setDescription("Stock status").setRequired(false),
        ),
    )
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((o) =>
          o.setName("kit").setDescription("Kit to look up").setRequired(true).setAutocomplete(true),
        ),
    )
    .addSubcommand((sub) =>
      sub
  ...
  async autocomplete(interaction) {
    ...
    if (focused.length === 0) { await interaction.respond([]); return; }

    const results = await KitService.searchKits(focused, 5);
    await interaction.respond(results.map((k) => ({ name: k.product_name, value: k.id })));
  },
  ...
  async execute(interaction) {
    ...
    if (subcommand === "add") {
      try {
        ...
        await interaction.reply(`✅ Added **${kit.product_name}** to the catalogue.`);
      } catch (err: any) {
    ...
    if (subcommand === "info") {
      ...
      if (!kit) {
        await interaction.reply({ content: "Kit not found.", flags: MessageFlags.Ephemeral });
        return;
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          { name: "Release Date", value: kit.release_date ?? "Unknown",               inline: true },
          { name: "Stock Status", value: kit.stock_status ?? "Unknown",               inline: true },
          { name: "Weight",       value: kit.weight_grams ? `${kit.weight_grams}g` : "Unknown", inline: true },
          { name: "Wishlisted",   value: `${kit.wishlistCount} member${kit.wishlistCount !== 1 ? "s" : ""}`, inline: true },
        );
    ...
    if (subcommand === "wanted") {
      ...
      if (wishlisters.length === 0) {
        await interaction.reply(`Nobody has **${kit.product_name}** on their wishlist yet.`);
        return;
      }
